...
//: 
func calculate(_ args: [String]) -> Int { // return an array maybe a loop?
    // insertion example: (["1", "+", "2"]) returns 2+2 or 4
    // loop through the array
    // grab the i[0] and i[2] turn into int
    // i[1] not sure yet
    if let last = args.last {
        switch last {
            case "count":
                var count = 0
                for value in args.dropLast() {
                    if Int(value) != nil {
                        count += 1
                    }
                }
...
func calculate(_ arg: String) -> Int { // return a single string
    let substrings = arg.split(separator: " ")
    var parts: [String] = []
...
func calculate(_ args: [String]) -> Double {
    ...
    if (args.count == 3){
        ...
        if let l = Double(left), let r = Double(right) {
            ...
            case "*": return l * r
            case "/": return l / r
            case "%": return l.truncatingRemainder(dividingBy: r)
            default: return 0.0
            }
        }
    }
...
func calculate(_ arg: String) -> Double {
    ...
    if let last = parts.last, last == "count" {
        var count = 0
        for value in parts.dropLast() {
            if Double(value) != nil {
                count += 1
            }
        }
